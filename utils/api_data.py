...
class Fss:
    ...
    def deposit_search(
        ...
        concat = []
        while True:
            url = f" http://finlife.fss.or.kr/finlifeapi/depositProductsSearch.json?auth={self.api_key}&topFinGrpNo={fin_grp_code}&pageNo={page_no}"
...
OUT1 = OUT_DIR / "fss.xlsx"
load_dotenv()
FSS_KEY = os.getenv("FSS_KEY")

def fss_data(OUT1, FSS_KEY):
    fss = Fss(FSS_KEY)
...
OUT2 = OUT_DIR / "ecos_interest.xlsx"
ECOS_KEY = os.getenv("ECOS_KEY")  
def interest_data(OUT2, ECOS_KEY):
    interest = [["산금채", "721Y001", "M", "6050000", 100],
    ...
    with pd.ExcelWriter(OUT2) as writer:
        ecos = Ecos(ECOS_KEY)
        for name, state_code, freq, item_code1, limit in interest:
            resp = ecos.stat_search(stat_code=state_code, freq=freq, item_code1=item_code1, limit=limit)
...
if __name__ == "__main__":
    fss_data(OUT1, FSS_KEY)
    interest_data(OUT2, ECOS_KEY)
